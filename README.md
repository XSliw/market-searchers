...
- **Kufar** — два режима на подписку (поле `mode`):
  - `all` (по умолчанию) — **любое новое** объявление по запросу, с **фото и кнопкой**
    «Открыть объявление». Много лотов, без медианы. Первый прогон подписки не спамит
    всё разом: запоминает базовую линию и шлёт только то, что появится потом. Потолок —
    10 уведомлений на подписку за прогон (защита от вала).
  - `deals` — только дешевле обрезанной по IQR медианы на `threshold_pct` (медиана
    считается, срезав выбросы по IQR, чтобы запчасти и случайные дорогие лоты её не тянули;
    лот — сделка, если дешевле медианы на `threshold_pct` **и** не ниже нижней границы IQR).
...
```json
{
  "paused": false,
  "subscriptions": [
    { "id": "iphone13", "source": "kufar", "enabled": true, "mode": "all",
      "label": "iPhone 13", "query": "iphone 13",
      "cat": "17010", "rgn": "7", "min_byn": 50, "max_byn": 5000 },
    { "id": "polar", "source": "kufar", "enabled": true, "mode": "all",
      "label": "Часы Polar", "query": "часы polar",
      "rgn": "7", "min_byn": 5, "max_byn": 5000 },
...
Kufar-поля: `mode` — `all` (все новые, с фото) или `deals` (дешевле медианы на
`threshold_pct`); `label` — подпись в строке «🔍 Поиск:» (иначе берётся `query`);
`cat` — категория (`17010` = мобильные телефоны), `rgn` — область
...
(в режиме `all` по умолчанию выкл, шлём и частников, и компании).
