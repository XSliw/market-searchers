...
```json
{
  ...
  "subscriptions": [
    ...
    { "id": "gpu", "source": "kufar", "enabled": true, "notify": "good",
      "label": "Видеокарты", "query": "видеокарта", "rgn": "7", "tag": "видеокарты", "min_deal_byn": 120 },
    { "id": "switch", "source": "sendico", "enabled": false, "query": "nintendo switch",
      "max_price_yen": null }
  ]
}
```
...
хэштег категории на карточках 🔥/🟢 (для поиска в чате); `min_deal_byn` — порог
«мелочи» в рублях: дешевле лот считается ⚪️ рынком, а не сделкой (по умолчанию 20;
задирай в шумных категориях — видеокарты 120, компы 400 — чтобы 🔥 не срабатывал на
дешёвом хламе); `cat` — категория (`17010` = мобильные телефоны), `rgn` — область
(**7** = Минск, **1** = Брестская, **2** = Гомельская, **3** = Гродненская,
**4** = Могилёвская, **5** = Витебская, **6** = Минская область). Цена **не**
ограничена — ищем во всех ценовых категориях; при желании можно задать
`min_byn`/`max_byn` в рублях. `private_only` — только частники (по умолчанию выкл,
шлём и частников, и компании).

Править можно прямо из Telegram: `/menu` рисует кнопки (пауза, вкл/выкл подписки,
переключение `notify` у Kufar-подписок, ±5000 ¥ потолка Sendico) и «🔥 Супервыгодные».
Изменение коммитит `config.json`, и следующий прогон (≤ 20 мин) его подхватывает.
